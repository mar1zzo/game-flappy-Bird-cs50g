...
-- goal4: declaration of local variable to control the pause feature
local pause = true
...
function love.keypressed(key)
    ...
    -- By Mariana Rizzo (dt: 2021/04):
    -- goal4: add the pause key, including music pause
    if key == 'p' then
        if pause == true then
            pause = false
            sounds['pause']:play()
...
function love.draw()
    ...
    -- By Mariana Rizzo (dt: 2021/04):
    -- goal4: draws a pause message on the screen informing the pause return key
    if pause == false then       
        love.graphics.setFont(flappyFont)
